...
var debug = false
var threshold = time.Duration(100 * time.Millisecond)
...
func (m *loggedMutex) Unlock() {
	...
	if duration >= threshold {
		fmt.Println("Mutex held for %v. Locked at %s unlocked at %s", duration, m.lockedAt, getCaller())
	}
	m.Mutex.Unlock()
}
...
func (m *loggedRWMutex) Lock() {
	...
	m.lockedAt = getCaller()
	if duration > threshold {
		fmt.Println("RWMutex took %v to lock. Locked at %s. RUnlockers while locking: %s", duration, m.lockedAt, strings.Join(m.unlockers, ", "))
	}
...
func (m *loggedRWMutex) Unlock() {
	duration := time.Now().Sub(m.start)
	if duration >= threshold {
		fmt.Println("RWMutex held for %v. Locked at %s: unlocked at %s", duration, m.lockedAt, getCaller())
	}
	m.RWMutex.Unlock()
}
...
func (wg *loggedWaitGroup) Wait() {
	...
	if duration >= threshold {
		fmt.Println("WaitGroup took %v at %s", duration, getCaller())
	}
}
